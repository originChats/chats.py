...
class Gateway:
    ...
    async def login(self):
        user_info = requests.get(f"https://api.rotur.dev/get_user?username={self.client.user}&password={hashlib.md5(self.client.password.encode()).hexdigest()}")
        if user_info.status_code != 200:
            raise Exception("Failed to authenticate with the gateway")
        
        user_key = user_info.json().get("key")
        validator = requests.get(f"https://api.rotur.dev/generate_validator?key={self.validator_key}&auth={user_key}")
